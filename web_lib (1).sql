...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 25 Feb 2026 pada 01.58
...
INSERT INTO `books` (`kdbuku`, `judul`, `jenis`, `tahun_terbit`, `penulis`, `penerbit`, `stock`, `created_at`, `updated_at`) VALUES
('BK00000002', 'Prabogo Engine', 'Framework', '2025', 'Prabogo', 'Prabogo', 12, '2026-01-20 16:50:06', '2026-02-24 16:54:17'),
('BK00000003', 'Investasi The Bahlil Way', 'Studi', '2025', 'Dadan Kurniawan', 'Bahlil Lahadalia', 15, '2026-02-23 19:02:29', '2026-02-23 19:02:29');
...
('DjLczJ9nIzQv3iOxpOL77dCWd5NpZYIfxMl755vX', NULL, '127.0.0.1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36', '', 1771981076),
('ihnfWPxw1Xp8HkFP3KEseVqk3UKjDij7WvuB7DuC', NULL, '127.0.0.1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36', '', 1771902005);
...
--
ALTER TABLE `borrowers`
  ADD CONSTRAINT `borrowers_ibfk_1` FOREIGN KEY (`id_peminjam`) REFERENCES `borrowings` (`id_peminjam`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Ketidakleluasaan untuk tabel `borrowings`
--
ALTER TABLE `borrowings`
  ADD CONSTRAINT `borrowings_ibfk_1` FOREIGN KEY (`no_pinjam`) REFERENCES `detail_borrowings` (`no_pinjam`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Ketidakleluasaan untuk tabel `detail_borrowings`
--
ALTER TABLE `detail_borrowings`
  ADD CONSTRAINT `detail_borrowings_ibfk_1` FOREIGN KEY (`kdbuku`) REFERENCES `books` (`kdbuku`) ON DELETE CASCADE ON UPDATE CASCADE;
